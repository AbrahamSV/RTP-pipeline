...
for nt=1:height(tracts)
    ...
    if exist(ts.fpath,'file')
       fprintf('[RTP_TractsGet] Using existing one because force=false\n')
        ...
        if ts.dilroiexc1>0;dilexc1=strcat("_dil-",num2str(ts.dilroiexc1));else;dilexc1="";end 
        if ts.dilroiexc2>0;dilexc2=strcat("_dil-",num2str(ts.dilroiexc2));else;dilexc2="";end 
        ...
        if ~(strcmp(ts.roiexc1,"NO"))
            roiexc1 = join(["-exclude", fullfile(string(ROIs_dir), ...
                    strcat(ts.roiexc1,dilexc1,ts.extroiexc1))]);
        end
        if ~(strcmp(ts.roiexc2,"NO"))
            roiexc2 = join(["-exclude", fullfile(string(ROIs_dir), ...
                    strcat(ts.roiexc2,dilexc2,ts.extroiexc2))]);
        end   
        ...
        if ts.wbt
           ...
           cmd       = join(["tckedit -quiet ", ...
                        "-include",roi1, "-include",roi2, roi3, roi4, roiexc1, roiexc2, ...
